...
def _platform_index_lowering(
  ...
  if len(platforms) == 1:
    ...
    if rule is None:
      raise NotImplementedError(
          f"No mosaic platform indexing rule found for platform {platforms[0]}")
  ...
  if platforms_without_specific_rule:
    if default_rule is None:
      raise NotImplementedError(
          f"Mosaic multi platform lowering on: {platforms} " 
          "missing default rule or exhaustive per-platform rules."
      )
  ...
  # note - for mosaic, we still want to honor backwards compatibility with
  # device specific rules, so we only apply the tpu or gpu rules if there is
  # no mosaic rule.
