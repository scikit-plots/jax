...
class LayoutTest(TestCase):
  ...
  def test_upcast_to_wgmma(self):
    dtype = jnp.int8
    swizzle = 128
    in_col_tiling = swizzle // 1
    m, n = 128, in_col_tiling * 2
    in_tiling = (64, in_col_tiling)
    out_col_tiling = swizzle // 2
    out_tiling = (64, out_col_tiling)
    tiled_layout_4 = fa._tiled_wgmma_layout_for_upcast((m, n))
    tiled_layout_2 = fa._tiled_wgmma_layout((m, n))
    def kernel(ctx, in_, out, smems):
      smem_in, smem_out, barrier = smems
      ctx.async_copy(src_ref=in_, dst_ref=smem_in, swizzle=swizzle, barrier=barrier)
      barrier.wait()
      t = mgpu.FragmentedArray.load_tiled(
          smem_in, swizzle=swizzle, is_signed=True, layout=tiled_layout_4
      )
      t = t.astype(ir.IntegerType.get_signless(16), is_signed=True)
      t = t.to_layout(tiled_layout_2)
      t.store_tiled(smem_out, swizzle=swizzle)
      mgpu.commit_shared()
      ctx.async_copy(src_ref=smem_out, dst_ref=out, swizzle=swizzle)
      ctx.await_async_copy(0)
    x = jax.random.randint(
        jax.random.key(42), tile_shape((m, n), in_tiling), -128, 127, dtype=dtype
    )
    y = jnp.moveaxis(x.astype(jnp.int16), -2, -3)
    y = y.reshape(m // out_tiling[0], out_tiling[0], n // out_tiling[1], out_tiling[1]).swapaxes(-2, -3)
    f = mgpu.as_gpu_kernel(
        kernel, (1, 1, 1), (128, 1, 1), x, y, [x, y, mgpu.TMABarrier()],
    )
